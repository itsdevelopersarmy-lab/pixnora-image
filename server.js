...
// Ensure role column exists BEFORE any queries
try {
...
async function startServer() {
  ...
  const bootstrapAdmin = async () => {
    ...
    if (!existingAdmin) {
      ...
      const result = stmt.run('Admin', adminEmail, '0000000000', hashedPassword, 'admin');
      const userId = result.lastInsertRowid;
      
      const initialKey = generateEncryptedKey();
      db.prepare('INSERT INTO api_keys (user_id, key, name) VALUES (?, ?, ?)').run(userId, initialKey, 'Admin Key');
    } else {
      // Update existing admin password and role just in case
  ...
  // Endpoint Cloning
  apiRouter.post('/endpoints/clone', authenticateToken, isAdmin, (req, res) => {
  ...
  apiRouter.get('/endpoints', authenticateToken, isAdmin, (req, res) => {
    const endpoints = db.prepare('SELECT * FROM cloned_endpoints WHERE user_id = ?').all(req.user.id);
    res.json(endpoints);
  });

  apiRouter.delete('/endpoints/:id', authenticateToken, isAdmin, (req, res) => {
    const { id } = req.params;
  ...
  app.all('/pixnora/*', async (req, res) => {
    ...
    // Check for proxy auth ONLY in apikey
